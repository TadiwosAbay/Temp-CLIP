...
class VideoQADataset(Dataset):
    def __init__(
        ...
        self.data = pd.read_csv("/NExT-GQA/datasets/nextqa/val.csv")
        ...
        base_feature_path = self.video_feature_path.replace("nextgqa", "").rstrip("/")
        new_subdirectory = "nextqa/CLIPL/CLIPL/openai_clip-vit-large-patch14"
        feature_dir = os.path.join(base_feature_path, new_subdirectory)

        self.video_feature_path = feature_dir
        # Define the new subdirectory to append
        #new_subdirectory = "nextqa/CLIPL/CLIPL/openai_clip-vit-large-patch14"
        
        available_ids = set(os.listdir(os.path.join(base_feature_path,new_subdirectory)))
        original_count = len(self.data)
        #//self.data = self.data[self.data['video_id'].astype(str).isin(available_ids)]
        filtered_count = len(self.data)
        print(f"Filtered {original_count - filtered_count} videos without features. Remaining: {filtered_count}")
        self.agu = False
        ...
        self.frame_feats = {}
         ...
         #if self.feat_type == 'DejaVu':
        self._load_dejavu_features()
        #else:
         #   print("SELECT THE Feat tyep properly")
            #self._load_hdf5_features()
    ...
    def numerical_sort(self,value):
        ...
        return 0  # Default fallback for unexpected 
    def _load_dejavu_features(self):
        """<--------------------Loads features from DejaVu .npz files.------------------------------------------->"""
        #print(f'Loading DejaVu features from {self.video_feature_path}...')
        base_feature_path = self.video_feature_path.replace("nextgqa", "").rstrip("/")

        # the new subdirectory to append
        new_subdirectory = "CLIPL/CLIPL/openai_clip-vit-large-patch14"
        print("base feature path is ", base_feature_path)
        print("new subdirectory is ", new_subdirectory)
        # Loop through the videos
        i=0
        #for video_id in sorted(os.listdir(os.path.join(base_feature_path, new_subdirectory))):
        for video_id in os.listdir(self.video_feature_path):
            video_path = os.path.join(self.video_feature_path, video_id)
            ...
            if(i<2):
                print(" video_path ", video_path)
                i=i+1
            if(video_id=="print.py"):
                continue
            if(video_id=="4010069381"):
                print("video was in the loop")
            # Combine all frames for this video
            frames = []
            frame_files = sorted(
            ...
            for frame_file in frame_files:
                if frame_file.endswith('.npz'):
                    frame_path = os.path.join(video_path, frame_file)
                    data = np.load(frame_path)
                    frames.append(data['embeddings']) 
            ...
            if len(frames) > 0:
                self.frame_feats[video_id] = np.stack(frames)  # Shape: (num_frames, feature_dim)
        if "4010069381" in self.frame_feats:  # Replace with "4010069381" if the key is a string
            print("Is it here?")
            print(self.frame_feats["4010069381"])
        else:
            print("Key not found in self.frame_feats")
            
        #print(f'Loading DejaVu features from {os.listdir(os.path.join(base_feature_path, new_subdirectory))}')
        print(f'Loaded {len(self.frame_feats)} videos.')
...
def get_videoqa_loaders(args, features_path, a2id, tokenizer, test_mode):
    ...
    if test_mode != 'train':
        ...
        print("CSV_PATH is ",test_mode == 'val')
        print("features PATH is ",features_path)
        print("CSV_PATH 1 is ",args.val_csv_path)
        print("CSV_PATH 2 is ",args.test_csv_path)
