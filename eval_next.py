...
def accuracy_metric(sample_list, result):
    ...
    for qtype, qns_ids in group.items():
        ...
        acc = 0
        for qid in qns_ids:
            # if qid not in preds:  # Skip if qid is not in preds
            #     print("HAPPENED")
            #     continue
            # if 'answer' not in preds[qid] or 'prediction' not in preds[qid]:  # Ensure necessary keys exist
            #     print("HAPPENED")
            #     continue
            cnt += 1
...
if __name__ == "__main__":
    ...
    parser.add_argument("--mode", type=str, default='val', help="Mode is fixed to 'val'")

    parser.add_argument("--folder", type=str)
    args = parser.parse_args()
    #res_dir = '../../../data/gmodels/NG+/TempCLIP/'+args.folder
    #res_dir ='../../../data/gmodels/NG+/TempCLIP/TempCLIP/val-res.json'
    #res_dir = '../data/models/nextqa/'
    mode = args.mode
    model_prefix = 'res'
    ##result_file = '{}/{}-{}.json'.format(res_dir, mode, model_prefix)
    result_file='../../../data/gmodels/NG+/TempCLIP/val-res.json'
