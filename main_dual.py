...
def main(args):
    ...
    # Evaluate
        test_acc, results, ground = eval(model, test_loader, a2v, args, test=False)
        ...
        if args.baseline in ['NG', 'NG+']:
            ...
            import eval_next
            eval_next.main(args.save_dir, osp.basename(gd_path), True)
        if args.baseline == 'posthoc':
            att_path = osp.join(args.save_dir, f'{args.test}_ground_att.json')
            save_to(att_path, ground_att)
            print("THE GD PATH IS ",att_path)
            from tools import getspan
            getspan.main(args.save_dir, args.test)
            import eval_next
            eval_next.main(args.save_dir, f'{args.test}_ground_ada.json')
